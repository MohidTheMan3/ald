...
void actuateHeatingElements()
{
  if (tc_active)
  {
    ...
    if (tc4_avg < (temp_sp2 - HYSTERESIS_TC2) && !relay2_on)
    {
      digitalWrite(RELAY4_PIN, LOW);   // Turn heater ON
      relay2_on = true;
    }
    else if (tc4_avg >= (temp_sp2 + HYSTERESIS_TC2) && relay2_on)
    {
    ...
    if (tc2_avg < (temp_sp4 - HYSTERESIS_TC4) && !relay4_on)
    {
      digitalWrite(RELAY2_PIN, LOW);   // Turn heater ON
      relay4_on = true;
    }
    else if (tc2_avg > (temp_sp4 + HYSTERESIS_TC4) && relay4_on)
    {
